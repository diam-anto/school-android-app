...
public class Questionnaire2 {
    ...
    private static Questionnaire2 Instance = null;
    private static Context Cont;

    public static Questionnaire2 GetInstance (Context Co) // leitoyrgia enos singleton
    {
        Cont = Co;
        if (Instance == null)
            Instance = new Questionnaire2();
        return Instance;
    }
    ...
    private void LoadDatabase ()
    {
        ...
        System.out.println ("*** !!!!!");
        try (BufferedReader BR = new BufferedReader (new InputStreamReader(AssMan.open ("chapter2.txt"))))
        {
            System.out.println ("*** -----");
            ...
            for (int i = 0; i < NoQ; i++)
            {
                ...
                int NoA = Integer.parseInt (BR.readLine ()); // το σύνολο των διαθέσιμων απαντήσεων
                Q.setCorrectAns (Integer.parseInt (BR.readLine ())); // αριθμός σωστής απάντησης
                for (int j = 0; j < NoA; j++)
